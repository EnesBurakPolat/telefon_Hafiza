...
# Thread ile video oluşturma işlemi başlatma
def on_create_video():
    try:
        ...
        if file_path:
            ...
            # Thread başlat
            thread = threading.Thread(target=create_video, args=(file_path, size_gb, progress_var, progress_label))
            thread.start()
...
tk.Label(root, text="Boyut (Bytes):", fg="blue", bg="white").grid(row=0, column=0, pady=10, padx=10, sticky="w")
entry_bytes = tk.Entry(root, bg="white")
...
tk.Label(root, text="Boyut (GB):", fg="red", bg="white").grid(row=1, column=0, pady=10, padx=10, sticky="w")
entry_size = tk.Entry(root, bg="white")
entry_size.grid(row=1, column=1, pady=10, padx=10)

tk.Label(root, text="Dosya Adı:", fg="green", bg="white").grid(row=2, column=0, pady=10, padx=10, sticky="w")
entry_filename = tk.Entry(root, bg="white")
